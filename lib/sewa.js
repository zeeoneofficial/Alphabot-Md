...
const expiredCheck = (haruka, _dir) => {
    setInterval(async() => {
        ...
        if (position !== null) {
            ...
            await haruka.sendMessage(_dir[position].id, { text: `Masa sewa di grup ini telah habis, hubungi owner untuk sewa lagi!` })
            await haruka.groupLeave(_dir[position].id)
